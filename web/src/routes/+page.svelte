...
<div class="space-y-8">
  ...
  <div class="text-center py-12">
    ...
    <p class="text-text-dim text-lg max-w-xl mx-auto">
      Sync your podcasts across all your devices. Open, self-hosted, compatible with AntennaPod, gPodder, and Kasts.
    </p>
    ...
    {#if !auth.loggedIn}
      <div class="mt-6 flex gap-3 justify-center">
        <a
          href="/login"
          class="px-6 py-2.5 bg-brand text-bg font-medium rounded-lg hover:opacity-90 transition-opacity"
        >
          Sign in
        </a>
      </div>
    {:else}
      <div class="mt-6 flex gap-3 justify-center">
        ...
        <a
          href="/discover"
          class="px-6 py-2.5 bg-surface border border-border text-text rounded-lg hover:bg-surface-hover transition-colors"
        >
          Discover Podcasts
        </a>
      </div>
    ...
    <div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {#each topPodcasts as podcast}
          <div class="bg-surface border border-border rounded-lg p-4 hover:bg-surface-hover transition-colors cursor-pointer group">
            {#if podcast.logo_url}
              <img
                src={podcast.logo_url}
                alt={podcast.title}
                class="w-full aspect-square object-cover rounded-md mb-3 group-hover:opacity-90 transition-opacity"
                loading="lazy"
              />
            {:else}
